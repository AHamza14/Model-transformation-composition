...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exsl="http://exslt.org/common" xmlns:uFN="uFN">
    ...
    <xsl:template name="subject_interface_template">
        ...
        <xsl:call-template name="interface_template">
            ...
            <xsl:with-param name="interface_project_name" select="$container_project" />
            <xsl:with-param name="interface_fully_qualified_name">
                <xsl:value-of select="concat($container_project, '.DesignPatterns.ObserverDesignPattern.Subject')"/>
            </xsl:with-param>
