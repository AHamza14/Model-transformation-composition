...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exsl="http://exslt.org/common" xmlns:uFN="uFN">
    ...
    <xsl:template match="ProjectInfo" mode="infra_project_template">

        <xsl:variable name="project_description" select="ancestor::Project/@Description" />
        
        <xsl:variable name="infra_project_references">
            ...
            <xsl:call-template name="reference_template">
                ...
                <xsl:with-param name="reference_version">
                    <xsl:text>7.0.0</xsl:text>
                </xsl:with-param>
            </xsl:call-template>
        </xsl:variable>
        ...
        <xsl:call-template name="project_template">
            ...
            <xsl:with-param name="project_references">
                <xsl:copy-of select="exsl:node-set($infra_project_references)/node()"/>
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
</xsl:stylesheet>
