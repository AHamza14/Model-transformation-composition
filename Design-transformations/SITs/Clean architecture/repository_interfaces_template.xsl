...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exsl="http://exslt.org/common" xmlns:uFN="uFN">
    ...
    <xsl:template match="//Class[Stereotypes/Stereotype[@Name='Repository']]" mode="repository_interfaces_template">

        <xsl:variable name="class_name" select="@Name" />
        <xsl:variable name="class_id" select="@Id" />

        <xsl:variable name="repository_interface_name">
            <xsl:text>I</xsl:text>
            <xsl:value-of select="$class_name" />
            <xsl:text>Repository</xsl:text>
        </xsl:variable>

        <xsl:variable name="repository_interface_imports">
        ...
        <xsl:variable name="respository_interface_implements">
            <xsl:call-template name="implement_template">
                <xsl:with-param name="interface_name">
                    <xsl:text>IAsyncRepository&#60;</xsl:text>
        ...
        <xsl:variable name="repository_interface_operations">
        ...
        <xsl:call-template name="interface_template">
            ...
            <xsl:with-param name="interface_name">
                <xsl:value-of select="$repository_interface_name" />
            </xsl:with-param>
            ...
            <xsl:with-param name="interface_fully_qualified_name">
                <xsl:value-of select="concat($projectName, '.Interfaces.', $repository_interface_name)"/>
            </xsl:with-param>
            <xsl:with-param name="interface_imports">
                <xsl:copy-of select="exsl:node-set($repository_interface_imports)/node()"/>
            </xsl:with-param>
            <xsl:with-param name="interface_implements">
                <xsl:copy-of select="exsl:node-set($respository_interface_implements)/node()"/>
            </xsl:with-param>
            <xsl:with-param name="interface_operations">
                <xsl:copy-of select="exsl:node-set($repository_interface_operations)/node()"/>
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
</xsl:stylesheet>
