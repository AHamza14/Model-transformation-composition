...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exsl="http://exslt.org/common" xmlns:uFN="uFN" version="2.0">
  ...
  <xsl:function name="uFN:typeUMLElement">
    ...
    <xsl:function name="uFN:constructorSpecificationBase">
      <xsl:param name="context"/>
      <xsl:for-each select="$context/ModelChildren/Parameter">
        <xsl:text>c.</xsl:text>
    ...
    <xsl:function name="uFN:constructorBase">
      <xsl:param name="context"/>
      <xsl:for-each select="$context/ModelChildren/Parameter">
    ...
    <xsl:function name="uFN:operationParameterDeclaration">
      <xsl:param name="context"/>
      <xsl:if test="$context/@Direction = 'inout' or $context/@Direction = 'out'">ref<xsl:text>&#032;</xsl:text></xsl:if><xsl:value-of select="uFN:typeUMLElementName($context)"/><xsl:value-of select="uFN:UML-DotNet-DataType($context/Type//@Name)"/><xsl:text>&#032;</xsl:text><xsl:value-of select="uFN:first-lower($context/@Name)"/>
    </xsl:function>

    <xsl:function name="uFN:constructorSignature">
      <xsl:param name="context"/>
      <xsl:for-each select="$context/Attributes/Attribute">
        <xsl:value-of select="uFN:constructorParameterDeclaration(.)"/>
        <xsl:if test="position()!=last()">, </xsl:if>
      </xsl:for-each>
    </xsl:function>

    <xsl:function name="uFN:constructorParameterDeclaration">
      <xsl:param name="context"/>
      <xsl:value-of select="uFN:typeUMLElementName($context)"/><xsl:value-of select="$context/Type//@Name"/><xsl:text>&#032;</xsl:text><xsl:value-of select="uFN:first-lower($context/@Name)"/>
    </xsl:function>
  ...
  <xsl:function name="uFN:getFullyQualifiedName">
    <xsl:param name="context"/>

    <xsl:variable name="package_name" select="$context/ancestor::Package/@Name"/>
    <xsl:value-of select="concat($projectName, '.', $package_name, '.', $context/@Name)"/>
  </xsl:function>

  <xsl:function name="uFN:getClassExtends">
    <xsl:param name="context"/>
    <xsl:variable name="extends" >
      <xsl:for-each select="$context/Extensions/Extend" >
          <xsl:value-of select="text()"/>
          <xsl:if test="position()!=last()">, </xsl:if>
      </xsl:for-each>
      <xsl:for-each select="$context/Implementations/Implement" >
          <xsl:if test="$context/Extensions/Extend and position()=1">, </xsl:if>
          <xsl:value-of select="text()"/>
          <xsl:if test="position()!=last()">, </xsl:if>
      </xsl:for-each>
    </xsl:variable>
  ...
  <xsl:function name="uFN:attributeGetterSetter">
    <xsl:param name="context"/>
    <xsl:variable name = "ff">
      <xsl:if test="($context/@HasGetter = 'true') or ($context/@HasSetter = 'true')">
        <xsl:if test="$context/@HasGetter = 'false'"> { </xsl:if>
        <xsl:if test="$context/@HasGetter = 'true'"> { get;</xsl:if>
        <xsl:text> </xsl:text>
        <xsl:if test="$context/@HasSetter = 'false'">}</xsl:if>
        <xsl:if test="$context/@HasSetter = 'true'"> set; }</xsl:if>
      </xsl:if>
    </xsl:variable>
    <xsl:value-of select="$ff"/>
  </xsl:function>

  <xsl:function name="uFN:UML-DotNet-DataType">
    <xsl:param name="value"/>
    <xsl:choose>
      <xsl:when test="$value = 'boolean'">
      ...
      <xsl:when test="$value = 'Date'">
        <xsl:sequence select="'DateTimeOffset'"/>
      </xsl:when>
      <xsl:when test="$value = 'date'">
        <xsl:sequence select="'DateTimeOffset'"/>
      </xsl:when>
